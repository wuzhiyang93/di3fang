<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input placeholder="品牌名称" size="medium" v-model="listQuery.name" maxlength="20" style="width: 150px;"
                class="filter-item"/>
    ...
    >
      ...
      <el-table-column label="品牌LOGO">
        <template slot-scope="scope"><img :src="scope.row.brand.url" height="36" style="vertical-align: middle">
        </template>
      </el-table-column>
      ...
      <el-table-column label="品牌别名">
        <template slot-scope="scope">{{ scope.row.brand.nickName }}</template>
      </el-table-column>
      <el-table-column label="操作" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="passOne(scope.row.id)">通过</el-button>
          <el-button size="mini" type="danger" @click="refuseOneShow(scope.row.id)">拒绝</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  import {
    queryBrandToBeAudit,  //分页查询待审核品牌
    passBrandAudit, //通过品牌审核
    refuseBrandAudit, //拒绝品牌审核
    batchPassBrandAudit,//批量通过品牌审核
    batchRefuseBrandAudit,//批量拒绝品牌审核
  } from '@/api/brandsauditlist';
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryBrandToBeAudit(this.listQuery).then(response => {
      ...
      passOne(id) {
        this.$confirm('确定要通过该品牌审核吗？', '提示', {
          ...
          this.isProcess = true;
          passBrandAudit(id).then(response => {
      ...
      refuseOne() {
        ...
        this.isProcess = true;
        refuseBrandAudit(this.form).then(response => {
          this.isProcess = false;
          this.refuseDialogVisible = false;
          if (response == 1) {
            this.$message({
      ...
      passDialog() {
        ...
        this.$confirm('确定要通过该品牌审核吗？', '提示', {
          ...
          batchPassBrandAudit(param).then(response => {
            this.isProcess = false;
            if (response > 0) {
              this.$message({
                type: 'success',
      ...
      refuseBatch() {
        ...
        batchRefuseBrandAudit(param).then(response => {
          this.isProcess = false;
          this.refuseDialogVisible = false;
          if (response > 0) {
            this.$message({
