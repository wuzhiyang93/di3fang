  ...
  <div class="app-container">
    <el-form label-position="right" :model="form" :rules="rules" ref="form" label-width="100px"
      ...
      <el-form-item>
        ...
        <router-link to="/sitesetting/article/articlelist">
...
<script>
  ...
  import {addArticle, queryChildColumnListForArticleForAdd,} from '@/api/article';
  ...
  export default {
    ...
    created() {
      this.queryChildColumnListForArticleForAdd()
    },
    methods: {
      // 查询所有二级栏目列表
      queryChildColumnListForArticleForAdd() {
        queryChildColumnListForArticleForAdd().then(res => {
          this.columnlist = res;
        });
      },
      // 新增文章
          ...
          if (!this.isProcess && valid && this.validateLength()) {
            ...
            addArticle(this.form).then(res => {
              ...
              if (res == -1) {
                ...
                this.$router.push({path: '/sitesetting/article/articlelist'})
              } else {
                this.$message({
                  message: '未知错误',
                  type: 'error'
