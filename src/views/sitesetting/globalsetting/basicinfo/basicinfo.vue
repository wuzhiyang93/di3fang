    ...
    <el-form label-position="right" :model="form" :rules="rules" ref="form" label-width="150px"
      ...
      <el-form-item label="浏览器标签图标">
        <el-upload
          ...
          <img v-if="form.labelLog" :src="form.labelLog" style="max-height: 100px; display: block">
      ...
      <el-form-item label="商城LOGO">
        <el-upload
          ...
          name="image"
          accept=".jpeg,.png,.gif,.ico,.JPEG,.PNG,.GIF,.ICO">
          <img v-if="form.logo" :src="form.logo" style="max-height: 100px; display: block">
          <i v-else class="el-icon-plus avatar-uploader-icon"></i>
        </el-upload>
      </el-form-item>
      <el-form-item label="管理端登录LOGO">
        <el-upload
          class="avatar-uploader"
          style="width: auto; height: auto"
          :action="uploadApi"
          :show-file-list="false"
          :on-success="uploadAdminLogoSuccess"
          :headers="headers"
          name="image"
          accept=".jpeg,.png,.gif,.ico,.JPEG,.PNG,.GIF,.ICO">
          <img v-if="form.adminLogo" :src="form.adminLogo" style="max-height: 100px; display: block">
          <i v-else class="el-icon-plus avatar-uploader-icon"></i>
        </el-upload>
      </el-form-item>
      <el-form-item label="管理端LOGO">
        <el-upload
          class="avatar-uploader"
          style="width: auto; height: auto"
          :action="uploadApi"
          :show-file-list="false"
          :on-success="uploadAdminIndexLogoSuccess"
          :headers="headers"
          name="image"
          accept=".jpeg,.png,.gif,.ico,.JPEG,.PNG,.GIF,.ICO">
          <img v-if="form.adminIndexLogo" :src="form.adminIndexLogo" style="max-height: 100px; display: block">
          <i v-else class="el-icon-plus avatar-uploader-icon"></i>
        </el-upload>
      </el-form-item>
      <el-form-item label="商城登录页面图片">
        <el-upload
          class="avatar-uploader"
          style="width: auto; height: auto"
          :action="uploadApi"
          :show-file-list="false"
          :on-success="uploadSiteLoginPicSuccess"
          :headers="headers"
          name="image"
          accept=".jpeg,.png,.gif,.ico,.JPEG,.PNG,.GIF,.ICO">
          <img v-if="form.siteLoginPic" :src="form.siteLoginPic" style="max-height: 100px; display: block">
          <i v-else class="el-icon-plus avatar-uploader-icon"></i>
        </el-upload>
      </el-form-item>
      <el-form-item label="商家登录页面图片">
        <el-upload
          class="avatar-uploader"
          style="width: auto; height: auto"
          :action="uploadApi"
          :show-file-list="false"
          :on-success="uploadStoreLoginPicSuccess"
          :headers="headers"
          name="image"
          accept=".jpeg,.png,.gif,.ico,.JPEG,.PNG,.GIF,.ICO">
          <img v-if="form.storeLoginPic" :src="form.storeLoginPic" style="max-height: 100px; display: block">
          <i v-else class="el-icon-plus avatar-uploader-icon"></i>
        </el-upload>
      </el-form-item>
      ...
      updateBaseInfoSet() {
        this.$refs['form'].validate(valid => {
          if (valid && this.validateLength() && !this.isProcess) {
            ...
            this.isProcess = true;
            updateBaseInfoSet(this.form).then(res => {
