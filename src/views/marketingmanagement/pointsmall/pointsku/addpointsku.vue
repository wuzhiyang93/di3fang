<template>
  <div class="app-container">
    <el-form label-position="right" :model="form" :rules="rules" ref="form" label-width="140px"
      ...
      <el-form-item>
        <el-button type="primary" @click="addPointSku">保存积分商品</el-button>
...
<script>
  ...
  import {queryAllPointCatesForAdd, addPointSku} from '@/api/addpointsku';
  ...
  export default {
    ...
    created() {
      this.queryAllPointCatesForAdd()
    },
    methods: {
      // 查询所有积分商城分类列表
      queryAllPointCatesForAdd() {
        queryAllPointCatesForAdd().then(res => {
          this.pointcates = res;
        });
      },
      // 放大预览图片
      ...
      addPointSku() {
        ...
        this.$refs['form'].validate(valid => {
          if (!this.isProcess && valid && this.validateLength()) {
            ...
            this.isProcess = true;
            addPointSku(this.form).then(res => {
