<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <br/>
      <router-link style="margin-left: 10px" to="/marketingmanager/goodsmarketing/addcrowdfunding">
        <el-button class="filter-item" size="medium" type="primary" icon="el-icon-plus">添加众筹项目</el-button>
      </router-link>
    </div>
    ...
    >
      ...
      <el-table-column label="操作" align="left" width="190">
        <template slot-scope="scope">
          <router-link v-if="getStatus(scope.row)!='1'"
                       :to="{path:'/marketingmanager/goodsmarketing/crowdfundingorders',query:{id:scope.row.id}}">
            <el-button type="success" style="margin: 0 5px 0 0" size="mini">查看订单</el-button>
          </router-link>
          <router-link v-if="getStatus(scope.row)=='1'"
                       :to="{path:'/marketingmanager/goodsmarketing/updatecrowdfunding',query:{id:scope.row.id}}">
            <el-button type="primary" style="margin: 0 5px 0 0" size="mini">修改</el-button>
          </router-link>
          <router-link v-if="getStatus(scope.row)!='1'"
                       :to="{path:'/marketingmanager/goodsmarketing/crowdfundingdetail',query:{id:scope.row.id}}">
            <el-button type="success" style="margin: 0" size="mini">查看详情</el-button>
          </router-link>
          <el-button v-if="getStatus(scope.row)=='1'" size="mini" style="margin: 0" type="danger"
                     @click="handleDelete(scope.row.id)">删除
          </el-button>
          <router-link :to="{path:'/marketingmanager/goodsmarketing/crowdfundingschedule',query:{id:scope.row.id}}">
...
<script>
  ...
  import {queryCrowdFundingMarketList, deleteCrowdFundingById} from '@/api/crowdfunding';
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryCrowdFundingMarketList(this.listQuery).then((response) => {
      ...
      delCrowdfunding(id) {
        ...
        this.isProcess = true;
        deleteCrowdFundingById(id).then(() => {
