...
<script>
  ...
  import {
    ...
    addCommunityBuy, //创建社区团购活动
  } from '@/api/addcommunitygroupbuy'
    ...
    methods: {
      ...
      prepareEditSkuInfo(row) {
        ...
        for (let i = 0; i < this.selectSkuArry.length; i++) {
          if (this.selectSkuArry[i].skuId == row.skuId) {
            ...
            console.log(this.selectSkuArry[i].mobileDesc);
      ...
      checkedChange(selection, row) {
        ...
        if (index > -1) {
          ...
          // for (let i = 0; i < this.selectSkuArry.length; i++) {
          //   if (this.selectSkuArry[i].skuId == row.id) {
      ...
      createData() {
        this.$refs['communityBuyForm'].validate(valid => {
          if (valid) {
            ...
            this.communityBuyForm.startTime = formatDate(this.communityBuyForm.startEndTime[0], 'yyyy-MM-dd hh:mm:ss');
            this.communityBuyForm.endTime = formatDate(this.communityBuyForm.startEndTime[1], 'yyyy-MM-dd hh:mm:ss');
            this.communityBuyForm.deliveryTime = formatDate(this.communityBuyForm.deliveryTime, 'yyyy-MM-dd hh:mm:ss');
            this.communityBuyForm.communityBuySkus = this.selectSkuArry;
            addCommunityBuy(this.communityBuyForm).then(response => {
              if (response > 0) {
                this.$message({
                  type: 'success',
                  message: '保存促销活动成功!'
