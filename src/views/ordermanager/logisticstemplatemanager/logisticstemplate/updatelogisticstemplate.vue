<template>
  <div class="app-container">
    ...
    <el-form :model="form" :rules="logisticsRules" ref="logisticsform" label-position="right" label-width="150px"
      ...
      <el-form-item>
        <router-link to="/ordermanager/logisticstemplatemanager/logisticstemplate">
    ...
    <el-dialog @opened="open()" title="添加收货地区" :visible.sync="dialogFormVisible">
    ...
    <el-dialog @opened="openFreeShip()" title="添加包邮条件" :visible.sync="freeshipShowFlag">
...
<script>
  import {
    queryAllProvincesWithCity,
    queryLogisticsTemplateById,
    updateLogisticsTemplate
  } from '@/api/updatelogisticstemplate';
  ...
  export default {
    ...
    methods: {
      // 根据物流模版id查询物流模版信息
      queryById(id) {
        queryAllProvincesWithCity().then(provinces => {
          this.provinces[0].child = JSON.parse(JSON.stringify(provinces));
          this.freeShipProvinces[0].child = JSON.parse(JSON.stringify(provinces));
          queryLogisticsTemplateById(id).then(res => {
            // 默认运费
            const defaultShippingMethod = res.freightBear == '0' ? res.shippingMethods.filter(item => item.isDefault == '1')[0] : null;
            this.form = {
              id: res.id,
              companyId: res.companyId, //物流公司id
              freightBear: res.freightBear, //谁承担运费 0 买家 1 商家 默认 0 买家
              isDefault: res.isDefault,//是否默认模版 0 是 1 否 默认1
              name: res.name, //物流模版名称
              pricintMethod: res.pricintMethod, //计价方式 0 按件  1 按重量  默认0
              shippingMethods: [
                {
                  first: defaultShippingMethod ? defaultShippingMethod.first : '1',
                  firstPlu: defaultShippingMethod ? defaultShippingMethod.firstPlu : '1',
                  isDefault: '1',
                  moenyPlu: defaultShippingMethod ? defaultShippingMethod.moenyPlu + '' : '0',
                  money: defaultShippingMethod ? defaultShippingMethod.money + '' : '0'
                }
              ],
            }

            // 设置运费方式
            if (res.shippingMethods.filter(item => item.isDefault == '0') && res.shippingMethods.filter(item => item.isDefault == '0').length > 0) {
              res.shippingMethods.filter(item => item.isDefault == '0').forEach(item => {
                // 运费方式
                const shippingMethod = {
                  areaNames: item.cityNames,
                  first: item.first,
                  firstPlu: item.firstPlu,
                  isDefault: '0',
                  moenyPlu: item.moenyPlu,
                  money: item.money,
                  shippingMethodAreas: [],
                  index: item.id,
                }
                ...
                // 设置运费方式的地区
                if (item.shippingMethodAreas && item.shippingMethodAreas.length > 0) {
                  item.shippingMethodAreas.forEach(area => {
                    shippingMethod.shippingMethodAreas.push({cityId: area.cityId});
                  })
                }
            ...
            // 设置包邮运费
            res.shippingMethodFreeShips.forEach(item => {
              // 包邮运费方式
              const shippingMethodFreeShip = {
                areaNames: item.cityNames,
                type: item.type,
                num: item.num,
                money: item.money,
              ...
              // 设置运费方式的地区
              if (item.shippingMethodFreeShipAreas && item.shippingMethodFreeShipAreas.length > 0) {
                item.shippingMethodFreeShipAreas.forEach(area => {
                  shippingMethodFreeShip.shippingMethodFreeShipAreas.push({cityId: area.cityId});
                })
              }

              this.shippingMethodFreeShips.push(shippingMethodFreeShip)
            })
          })
        })
      },
      openFreeShip() {
        // 设置已经选择的地区不能选择
        if (this.shippingMethodFreeShips && this.shippingMethodFreeShips.length > 0) {
          this.shippingMethodFreeShips.forEach(shippingMethod => {
            shippingMethod.shippingMethodFreeShipAreas.forEach(area => {
              this.$refs.freeshipTree.getNode(area.cityId).data.disabled = true;
        ...
        if (this.shippingMethods && this.shippingMethods.length > 0) {
          this.shippingMethods.forEach(shippingMethod => {
            shippingMethod.shippingMethodAreas.forEach(area => {
              this.$refs.tree.getNode(area.cityId).data.disabled = true;
      ...
      saveLogistics() {
        this.$refs['logisticsform'].validate(valid => {
          if (valid && !this.isProcess) {
            ...
            updateLogisticsTemplate(this.form).then(res => {
              ...
              if (res > 0) {
                ...
                this.$router.push({path: '/ordermanager/logisticstemplatemanager/logisticstemplate'})
              } else if (res == -2) {
                this.$message({
                  type: 'error',
                  message: '运费模版名称已存在！'
