    ...
    >
      ...
      <el-table-column label="订单号" min-width="170">
        <template slot-scope="scope">
          <router-link :to="{path:'/ordermanager/ordermanager/backorder/orderdetail',query:{id:scope.row.orderId}}">
      ...
      <el-table-column label="操作" align="left" width="100">
        <template slot-scope="scope">
          <router-link v-if="showRefundDetail(scope.row)"
                       :to="{path:'/ordermanager/ordermanager/refundreview',query:{id:scope.row.id}}">
            <el-button type="success" size="mini">退款详情</el-button>
          </router-link>
          <router-link v-if="showReturnDetail(scope.row)"
                       :to="{path:'/ordermanager/ordermanager/returnreview',query:{id:scope.row.id}}">
            <el-button type="success" size="mini">退货详情</el-button>
          </router-link>
          <router-link v-if="showRefundAudit(scope.row)"
                       :to="{path:'/ordermanager/ordermanager/refundreview',query:{id:scope.row.id}}">
            <el-button type="primary" size="mini">退款审核</el-button>
          </router-link>
          <router-link v-if="showReturnAudit(scope.row)"
                       :to="{path:'/ordermanager/ordermanager/returnreview',query:{id:scope.row.id}}">
            <el-button type="primary" size="mini">退货审核</el-button>
          </router-link>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  import {queryBackOrders} from '@/api/backorders';
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        queryBackOrders(this.listQuery).then(res => {
          this.list = res.data;
      ...
      showReturnDetail(order) {
        return order.status == '6' || order.status == '5' || order.status == '8' || order.status == '9';
      },
      // 显示退款审核
      showRefundAudit(order) {
        return order.status == '1';
      },
      // 显示退货审核
      showReturnAudit(order) {
