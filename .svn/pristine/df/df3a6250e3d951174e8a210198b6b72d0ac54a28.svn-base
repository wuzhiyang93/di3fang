<template>
  <div class="app-container">
    <el-form label-position="right" :model="form" :rules="rules" ref="form" label-width="100px"
      ...
      <el-form-item>
        <el-button type="primary" @click="addHelp">保存</el-button>
        <router-link to="/sitesetting/help/helplist">
...
<script>
  ...
  import {queryAllHelpCategoryForAdd, addHelp} from '@/api/helplist';
  ...
  export default {
    ...
    created() {
      this.queryAllHelpCategoryForAdd()
    },
    methods: {
      // 查询所有帮助分类列表
      queryAllHelpCategoryForAdd() {
        queryAllHelpCategoryForAdd().then(res => {
          ...
          if (!this.isProcess && valid && this.validateLength()) {
            ...
            addHelp(this.form).then(res => {
              ...
              if (res == 1) {
                ...
                this.$router.push({path: '/sitesetting/help/helplist'})
              } else {
                this.$message({
                  message: '创建失败',
                  type: 'error'
