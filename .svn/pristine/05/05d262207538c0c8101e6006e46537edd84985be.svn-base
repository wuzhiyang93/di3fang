  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" align="left" min-width="185">
        <template slot-scope="scope">
          <router-link :to="{path:'/marketingmanager/goodsmarketing/crowdfundingorderdetail',query:{id:scope.row.id}}">
          ...
          <el-button v-if="scope.row.status == '1'" type="primary" style="margin: 0 10px 5px 0" size="mini"
                     @click="toConfirmOrder(scope.row.id)">确认付款
          </el-button>
          <el-button v-if="deliveryBtnShow(scope.row)" type="primary" style="margin: 0 10px 0 0" size="mini"
          ...
          <el-button v-if="scope.row.status == '1'" size="mini" style="margin: 0 10px 0 0" type="danger"
                     @click="toCancelOrder(scope.row.id)">取消
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deliveryBtnShow(row) {
        //如果是全款支持，或者被抽中
        return row.status == '2' && (row.orderType == '4' || (row.orderType == '5' && row.lotteryStatus == '1'));
      },
      // 确认付款二次确认
      toConfirmOrder(id) {
