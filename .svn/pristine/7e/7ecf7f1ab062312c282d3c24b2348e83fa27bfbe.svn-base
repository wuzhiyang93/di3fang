<template>
  <div class="app-container">
    <el-form :model="fullDown" :rules="rules" ref="form" label-position="right" label-width="120px"
             style="margin-top: 20px">
      <el-form-item prop="name">
        <span slot="label">促销名称</span>
        <el-input v-model="fullDown.name" maxlength="40"/>
      </el-form-item>
      <el-form-item>
        <span slot="label"><span class="labelImportant">*</span>促销规则</span>
        <el-tooltip class="item" effect="dark" content="最多添加三级" placement="right">
          <el-button class="filter-item" size="medium" type="primary" icon="el-icon-plus" @click="addFullPrice()">
            添加多级促销
          </el-button>
        </el-tooltip>
      </el-form-item>
      <el-form-item>
        ...
        >
          ...
          <el-table-column label="减金额">
            <template slot-scope="scope">
              <el-input v-model="scope.row.price" type="text" size="small" maxlength="20" style="width: 200px">
                <template slot="prepend">¥</template>
              </el-input>
            </template>
          </el-table-column>
          <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button v-if="scope.$index!=0" size="mini" type="danger"
                         @click.native.prevent="deleteFullPrice(scope.$index, fullDown.fullDowns)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-form-item label="促销描述">
        <el-input v-model="fullDown.desc" type="textarea" maxlength="2048" :rows="3"/>
      </el-form-item>
      <el-form-item>
      ...
      <el-form-item>
        <el-button type="primary" @click="saveFullDown()">保存促销活动</el-button>
        <router-link to="/marketingmanager/goodsmarketing/fulldown">
...
<script>
  ...
  import {
    querySkusForFullDown,
    addFullDownMarketing,
    queryFullDownMarketingById,
    updateFullDown,
    querySkusForFullDownUpdate
  } from '@/api/editfulldown';
  ...
  export default {
    ...
    methods: {
      queryFullDown() {
        queryFullDownMarketingById(this.id).then(res => {
          this.fullDown.id = res.id;
          this.fullDown.desc = res.desc;
          this.fullDown.name = res.name;
          this.fullDown.startTime = res.startTime;
          this.fullDown.endTime = res.endTime;
          this.fullDown.timeValue = [];
          this.fullDown.timeValue.push(res.startTime);
          this.fullDown.timeValue.push(res.endTime);
          ...
          this.fullDown.fullDowns = [];


          if (res.fullDowns && res.fullDowns.length > 0) {
            res.fullDowns.forEach(fullDown => {
              this.fullDown.fullDowns.push({'fullPrice': fullDown.fullPrice, 'price': fullDown.price});
            })
          }
        })
      },
      ...
      querySkus() {
        ...
        if (this.isUpdate()) {
          querySkusForFullDownUpdate(this.listQuery).then(res => {
            this.setSkus(res);
          })
        } else {
          querySkusForFullDown(this.listQuery).then(res => {
            this.setSkus(res);
          })
        }
      },
      ...
      addFullPrice() {
        // 最多只能添加3条满减数据
        if (this.fullDown.fullDowns.length < 3) {
          this.fullDown.fullDowns.push({'fullPrice': '', 'price': ''});
        }
      },
      deleteFullPrice(index, rows) {
        rows.splice(index, 1)
      },
      // 保存满减促销
      saveFullDown() {
        ...
        this.$refs['form'].validate(valid => {
          if (!this.isProcess && valid) {
            this.isProcess = true;
            this.fullDown.marketingSkus = [];
            this.fullDown.startTime = this.fullDown.timeValue[0];
            this.fullDown.endTime = this.fullDown.timeValue[1];
            this.selectedSkus.forEach(sku => {
              this.fullDown.marketingSkus.push({skuId: sku.id});
            })

            // 更新
            if (this.isUpdate()) {
              updateFullDown(this.fullDown).then(() => {
                this.isProcess = false;
                this.$message({
                  type: 'success',
                  message: '更新满减成功!'
                });
                this.$router.push({path: '/marketingmanager/goodsmarketing/fulldown'})
              })
            } else {
              // 新增
              addFullDownMarketing(this.fullDown).then(() => {
                this.isProcess = false;
                this.$message({
                  type: 'success',
                  message: '添加满减成功!'
