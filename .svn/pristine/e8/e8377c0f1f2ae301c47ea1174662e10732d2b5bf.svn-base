...
<script>
  ...
  import {
    queryStoreCrowdfundingMarketingById,
    queryMarketingCatesByTypeForStoreCrowdfunding
  } from '@/api/storecrowdfundingdetail';
  ...
  export default {
    ...
    methods: {
      // 查询众筹促销
      queryCrowdfunding() {
        queryStoreCrowdfundingMarketingById(this.$route.query.id).then(res => {
          ...
          queryMarketingCatesByTypeForStoreCrowdfunding().then(res => {
            if (res && res.length > 0) {
