  ...
  <div class="app-container">
    ...
    <div class="detailsTable">
      ...
      <el-row>
        <!-- <el-col :span="8">公司邮箱：{{storeInfo.companyEmail | isSubmit }}</el-col>
        <el-col :span="8">公司电话：{{storeInfo.companyPhone | isSubmit}}</el-col> -->
        <el-col :span="8">公司地址：{{storeInfo.companyDetailAddress | isSubmit}}</el-col>
        <el-col :span="8">详细地址：
            <div style="display:inline-block;position: relative;width: 300px;top: 0px;">
              <el-select v-model="storeInfo.provinceId" disabled @change="changeProvince" placeholder="选择省份" style="width:30%;float:left">
              <el-option
            ...
            <el-select v-model="storeInfo.cityId"  disabled @change="changeCity"  placeholder="选择城市" style="width:30%;position: absolute;right:35%">
              <el-option
            ...
            <el-select v-model="storeInfo.districtId" disabled @change="changeDistrict" placeholder="选择区域" style="width:30%;position: absolute;right:0">
              <el-option
        ...
        <el-col :span="8">联系方式：{{storeInfo.contactPhone | isSubmit}}</el-col>
      </el-row>
      <el-row>
        <el-col :span="8">公司联系人：{{storeInfo.contactPerson | isSubmit}}</el-col>
        <el-col :span="8">法人身份证正面
          <el-link
            v-show="showCarPic"
            :underline="false"
            type="success"
        ...
        <el-col :span="8">法人身份证反面：
          <el-link
            v-show="showCarPic"
            :underline="false"
            type="success"
            :href="storeInfo.cardPicBack"
            target="_blank"
            style="vertical-align: bottom"
          ><i class="el-icon-view el-icon--right"/> 查看
          </el-link>
          {{storeInfo.cardPicBack | noPhoto}}
        </el-col>
      </el-row>
      <el-row>
    ...
    <div class="detailsTable" style="padding: 10px 10px 5px 10px">
      ...
      <div v-if="threeCategoryList.length==0">暂无分类</div>
    </div>
...
<script>
  ...
  import {queryAllProvinces,queryDistrictByCityId,queryCityByProvinceId} from '@/api/addshopsecondstep';
    ...
    methods: {
      getList() {
        ...
        storeDetailInfoByStoreId(this.listQuery).then(response => {
          ...
          queryCityByProvinceId(this.storeInfo.provinceId).then((response) => {
            this.cityList = response;
          })
          queryDistrictByCityId( this.storeInfo.cityId).then((response) => {
